<template>
  <section class="msite">
    ...
    <!--首页导航-->
    <nav class="msite_nav" >
      <div class="swiper-container" v-if="FoodListArr.length>0">
        <div class="swiper-wrapper">
          <div class="swiper-slide" v-for="(foods,index) in FoodListArr" :key="index">
            <div class="link_to_food" v-for="(food,index) in foods" :key="index">
...
import chunk from 'lodash/chunk'  //只引入需要的包，这样打包的问价就小
  ...
  export default {
    computed:{
      ...mapState({
        ...
        FoodList:state=>state.msite.FoodList,
        ShopList:state=>state.msite.ShopList,
        user:state=>state.user.user,
        
      }),
      FoodListArr(){
        return chunk(this.FoodList,8)
      }
    },
    components:{
      Star
    ...
    async mounted(){
      //分发异步actions将后台数据请求到vuex中
      this.$store.dispatch('getAddress')
     /*  this.$store.dispatch('getFoodList',()=>{
        this.$nextTick(()=>{
         //Swiper对象必须在列表对象显示之后创建
